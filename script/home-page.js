...
const hoverTargets = document.querySelectorAll(
  "a, button, .project-card, .achievement-card"
);

hoverTargets.forEach(el => {
  el.addEventListener("mouseenter", () => {
    cursor.style.background = "radial-gradient(circle, var(--cursor-hover), transparent 70%)";
    cursor.style.filter = "blur(10px)";
  });

  el.addEventListener("mouseleave", () => {
    cursor.style.background = "radial-gradient(circle, var(--cursor-color), transparent 70%)";
  ...
  hoverTargets.forEach(el => {
    el.addEventListener("mouseenter", () => {
      cursor.style.background =
        "radial-gradient(circle, var(--cursor-hover), transparent 70%)";
      cursor.style.filter = "blur(10px)";
    });

    el.addEventListener("mouseleave", () => {
      cursor.style.background =
        "radial-gradient(circle, var(--cursor-color), transparent 70%)";
      cursor.style.filter = "blur(6px)";
    });
  });
});
...
function printTerminalLines() {
 ...
 output.textContent = "";
  ...
  // Start faster
  setTimeout(() => {
    typeWriterEffect("Arooshi Sharma's Portfolio", output, () => {
      setTimeout(() => {
